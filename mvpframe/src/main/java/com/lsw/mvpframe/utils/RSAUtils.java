...
public final class RSAUtils
{
    ...
    public static byte[] decryptByRSA1(byte[] pubKeyInByte, byte[] data) {
        try {
            KeyFactory mykeyFactory = KeyFactory.getInstance("RSA","BC");
            ...
            LogUtil.e("公钥解密失败"+e.getMessage().toString());
            e.printStackTrace();
            return null;
        }
    }

    public static byte[] decryptByRSAKey(PublicKey publicKey, byte[] data) {
        try {
//            KeyFactory mykeyFactory = KeyFactory.getInstance("RSA","BC");
//            X509EncodedKeySpec pub_spec = new X509EncodedKeySpec(pubKeyInByte);
//            PublicKey pubKey = mykeyFactory.generatePublic(pub_spec);
